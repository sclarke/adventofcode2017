...
def parse_line(line):
    """Convert string representing numbers to a sequence of integers"""
    return [int(v) for v in line.strip().split()]

def compute_part1_chksum(input_string):
    return sum(get_diff(parse_line(l)) for l in input_string.split('\n'))

print(f"Part 1 test checksum: {compute_part1_chksum(test_input)}")
print(f"Part 1 puzzle checksum: {compute_part1_chksum(input)}")
...
def get_divisor(iter):
    sorted_iter = sorted(iter)
    for num in reversed(sorted_iter):
        for den in sorted_iter:
            quotient = num/den
            remainder = num % den
            if quotient < 2:
                break
...
def compute_part2_chksum(input_string):
    return sum(get_divisor(parse_line(l)) for l in input_string.split('\n'))

print(f"Part 2 Test checksum: {compute_part2_chksum(part2_test_input)}")
print(f"Part 2 puzzle checksum: {compute_part2_chksum(input)}")